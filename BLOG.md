...
to spend years and hundreds of millions of dollars on one target. The steps between tiers are
...
grows over time: a scenario set in 2024 begins with fourteen controls partly in place, one set in
...
described is a facility, which understated the programme by a factor of about six.
...
The **do-nothing** scenario is not a flat line either, and that is the point of including it: a
posture that stands still gets worse, ending the decade meaningfully more exposed than it started,
purely because the attackers improved.
...
secure facility takes up to four years to build and accredit, and a custom accelerator takes about
the same, so what a lab has in place in 2030 is mostly determined by what it starts in the next year
or two. That holds whatever you believe about how fast AI improves. The reactive scenario says the
same thing from the other end, where every individual decision is correct, all of them are late, and
the money spent does not buy the time back.
...
embarrassment. Four entries claimed no lab had deployed something a lab had publicly described
deploying. The cost of confidential computing was out by roughly five times, in the direction that
made it look harder than it is. Two entries were the same control counted twice. And the facility,
the single largest line in the whole model, had been priced as a room. Most of the corrections made
the picture *better* than the version I had been carrying around, which is a useful reminder that a
model drifts pessimistic as easily as optimistic once you stop checking it.
...
nobody finds. That monotonic behaviour is asserted in the test suite, so when the constrained
